  ...
  useEffect(() => {
    if (filters.filterBy && filters.filterValue) {
      const filtered = applications.filter((app) =>
        app[filters.filterBy]?.toString().toLowerCase().includes(filters.filterValue.toLowerCase())
      );
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="p-6">
        ...
        <div className="flex justify-between mb-4">
          <h1 className="text-2xl font-semibold">Applications List</h1>
        </div>
          ...
          <tbody {...getTableBodyProps()} className="bg-white divide-y divide-gray-200">
            {page.map((row) => {
              ...
              return (
                <tr {...row.getRowProps()}>
                  {row.cells.map((cell) => (
                    <td {...cell.getCellProps()} className="py-2 px-4 text-sm font-medium text-gray-900">
                      {cell.render("Cell")}
                    </td>
                  ))}
                </tr>
